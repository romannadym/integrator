...
def ContactsListView(request):
    ...
    context = {'contacts': contacts, 'organization_id':request.user.organization_id}
    return render(request, 'accounts/contacts.html', context)

@login_required
def AddContactView(request,organization_id):
    form = OrganizationContactForm()

    context = {'form': form, 'action': reverse('api-contacts',  kwargs={'organization_id': organization_id})}
